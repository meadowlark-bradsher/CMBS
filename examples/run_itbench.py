"""ITBench-style incident triage against the unified Session kernel.

The adapter proposes checks, a scripted oracle reports their outcomes, and
each outcome becomes one ``probe_result`` op in the session's log.
"""
...
def run(scenario: dict[str, str] | None = None, kit_name: str = "itb_min_4") -> Session:
    kit = load_builtin_kit(kit_name)
    adapter = ITBenchAdapter(kit)
    oracle = ITBenchOracle(kit, scenario=scenario or SCENARIO)
